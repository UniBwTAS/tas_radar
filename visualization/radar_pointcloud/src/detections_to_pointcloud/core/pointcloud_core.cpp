...
PointCloudData Converter::build(const Input& in, const Transform& rearAxis, const Dynamics& dyn, const LogFn& log)
{
    ...
    /// Assemble the sensor-frame cloud
    PointCloudData out;
    out.stamp_ns = in.stamp_ns;
    out.frame_id = in.frame_id;
    out.points.reserve(in.detections.size());
    for (const Detection& d : in.detections)
    {
        PclPoint p;

        // Cartesian coordinates in the sensor frame
        p.x = d.range * std::cos(d.azimuth) * std::cos(d.elevation);
        p.y = d.range * std::sin(d.azimuth) * std::cos(d.elevation);
        p.z = d.range * std::sin(d.elevation);
        ...
        p.absolute_radial_speed = range_rate_absolute;
        p.rgb = color_hex_as_float;

        // Raw measurement
        p.range = d.range;
        p.azimuth = d.azimuth;
        p.elevation = d.elevation;
        p.range_rate = d.radial_speed;
        p.rcs = d.rcs;
        p.power = d.power;
        p.noise = d.noise;
