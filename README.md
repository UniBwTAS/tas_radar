> [!CAUTION]
> **This `rosx` branch is outdated.** The [**`master`** branch](https://github.com/UniBwTAS/tas_radar/tree/master) now supports **both ROS 1 and ROS 2** from a single source tree (selected at build time via `$ROS_VERSION`), across all packages. Please use `master` instead of this branch.
